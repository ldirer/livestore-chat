...
export function ChatView({user}: ChatViewProps) {
  ...
  const handleSubmit = React.useCallback(({content}: Pick<TMessage, 'content'>) => {
    store.commit(events.messageCreated({id: crypto.randomUUID(), content, createdAt: new Date(), authorId: user.id}))
  }, [store, user])
